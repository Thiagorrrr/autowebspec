      ...
      <div className="max-w-7xl mx-auto flex justify-between items-center">
        <div className="flex items-center gap-3">
          <div className="bg-[#6319F7] p-2.5 rounded-xl shadow-lg shadow-[#6319F7]/30 transform -rotate-3">
            <Link href="/" aria-label={`icon-home`}>
              <Flame className="text-white" size={20} fill="currentColor" />
            </Link>
          </div>
          <div>
            <Link href="/"><h1 className="text-xl font-black tracking-tighter text-gray-900">AUTO<span className="text-[#6319F7]">WEBSPEC</span></h1>
              <p className="text-[10px] text-black uppercase tracking-[0.2em] font-bold">Comparador Pro</p></Link></div>
        </div>
        <nav className="hidden md:flex gap-8 text-sm font-bold text-gray-400">
          ...
          <Link href="/produtos">
            <span
              className={`hover:text-[#6319F7] transition-colors ${pathname.startsWith("/produtos") ? "text-[#6319F7]" : ""
                }`}
            >
              PRODUTOS
            </span>
